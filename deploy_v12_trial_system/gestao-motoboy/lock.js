...
(function () {
    ...
    async function checkStatus() {
        ...
        if (!key) {
            ...
            // Mantemos a lógica antiga de localStorage 'first_run' se necessário,
            // mas o foco agora é forçar a entrada da chave (Trial ou Full).
        ...
        try {
            ...
            // Em caso de erro de rede, se tiver chave local, deixa passar (Fail-Open temporário ou Fail-Close?)
            // Para segurança: Fail-Open se já validou antes (cache), mas aqui simplificamos.
        }
    }
    ...
    function blockSystem(titleText, msgText) {
        // Força a tela de login mas com texto de bloqueio
        const login = document.getElementById('view-login');
        if (login) {
            login.classList.remove('hide');
            login.style.display = 'flex';
            ...
            if (p) {
                p.innerHTML = `${msgText}<br><br><a href="../loja/checkout.html?product=motoboy" class="text-blue-600 font-bold underline">COMPRAR VERSÃO VITALÍCIA</a>`;
            }
    ...
    function unlockSystem() {
        ...
        if (login) {
            ...
            // login.style.display = 'none'; // CSS class hide usually handles this
        }
        // Restaurar App
        const sidebar = document.getElementById('sidebar');
        const main = document.querySelector('main');
    ...
    function showTrialBanner(expDate) {
        ...
        if (diffHrs <= 0) return; // Deveria estar bloqueado
